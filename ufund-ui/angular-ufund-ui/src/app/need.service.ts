...
export class NeedService {
  ...
  private needsUrl = 'http://localhost:8080/cupboard/needs';  // URL to web api
  ...
  searchNeeds(term: string): Observable<Need[]> {
    ...
    return this.http.get<Need[]>(`${this.needsUrl}/search/?name=${term}`).pipe(
      tap(x => x.length ?
  ...
  shareNeed(need: Need): Observable<Need> {
    return this.http.post<Need>(`${this.needsUrl}/shareneed`, need, this.httpOptions).pipe(
      tap(() => this.log("shared need")), catchError(this.handleError<Need>('error'))
    );
  }
  ...
  updateNeed(need: Need): Observable<any> {
    return this.http.put(this.needsUrl, need, this.httpOptions).pipe(
      tap(_ => {this.log(`updated need id=${need.id}`); console.log("yooo");}),
